...
from b_team import run_market_map_slider  # (가능하면 파일 상단 import로 이동 추천)

with tab2:
    st.subheader("🗺️ 지역별 현황 (기간 필터 + 지역 선택 반영)")

    if selected_regions:
        # ✅ 기간 필터가 반영된 filtered_df를 넘김
        fig_b = run_market_map_slider(filtered_df, selected_regions)

        if fig_b is None:
            st.warning("⚠️ 선택된 지역이 없습니다. 사이드바에서 지역을 선택해주세요.")
        else:
            # ✅ key를 주면 선택 변화(x 삭제) 때 새로 그리기가 더 확실해짐
            st.plotly_chart(fig_b, use_container_width=True, key=f"bmap-{len(selected_regions)}")
    else:
